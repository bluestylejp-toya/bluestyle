    ...
    public function findProductIdsOrder(&$objQuery, $arrVal = array())
    {
        ...
        if (is_array($this->arrOrderData) and $objQuery->order == '') {
            ...
            if ($this->arrOrderData['col'] == 'count_of_favorite'){
                $objQuery->setOrder(" alldtl.status ASC, (SELECT COUNT(DISTINCT customer_id) FROM dtb_customer_favorite_products INNER JOIN dtb_customer USING (customer_id) WHERE dtb_customer_favorite_products.product_id = alldtl.product_id AND dtb_customer.del_flg = 0) DESC, product_id DESC");
            } else {
