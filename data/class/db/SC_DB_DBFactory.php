...
class SC_DB_DBFactory
{
    ...
    public function alldtlSQL($where_products_class = '', $product_ids = array())
    {
         ...
         *
         * 商品規格なしのみという前提で、集計処理を省いた。
         * *_min と *_max は常に同じ値となる。
         * https://bluestyle.backlog.jp/view/CHAIN-13#comment-115927495
         */
        $sql = <<< __EOS__
            (
                SELECT
                     dtb_products.*
                    ,T4.product_code AS product_code_min
                    ,T4.product_code AS product_code_max
                    ,T4.price01 AS price01_min
                    ,T4.price01 AS price01_max
                    ,T4.price02 AS price02_min
                    ,T4.price02 AS price02_max
                    ,T4.stock AS stock_min
                    ,T4.stock AS stock_max
                    ,T4.stock_unlimited AS stock_unlimited_min
                    ,T4.stock_unlimited AS stock_unlimited_max
                    ,T4.point_rate
                    ,T4.deliv_fee
                    ,dtb_maker.name AS maker_name
                    ,dtb_customer.pref
                    ,dtb_customer.nickname
                    ,(SELECT COUNT(distinct customer_id) FROM dtb_customer_favorite_products INNER JOIN dtb_customer USING (customer_id) WHERE product_id = dtb_products.product_id AND dtb_customer.del_flg = 0) AS count_of_favorite
                    ,(SELECT GROUP_CONCAT(distinct customer_id) FROM dtb_customer_favorite_products INNER JOIN dtb_customer USING (customer_id) WHERE product_id = dtb_products.product_id AND dtb_customer.del_flg = 0) AS favorite_customer_list
                FROM $dtb_products_table
                    INNER JOIN (
                        SELECT *
                        FROM dtb_products_class
                        WHERE del_flg = 0 AND classcategory_id1 = 0 AND classcategory_id2 = 0 $where_products_class $product_id_cause
                    ) AS T4
