...
class SendNotificationRequestInfo
{
    ...
    public function main($start, $end)
    {
        ...
        foreach ($arrRequestItemByCustomer as $requestItemByCustomer){
            ...
            $objPage->kana = $requestItemByCustomer[0]['source_kana01'] .' '. $requestItemByCustomer[0]['source_kana02'];
            $objPage->requestItemCount = count($requestItemByCustomer);
            $objPage->nickname = $requestItemByCustomer[0]['source_nickname'];
            $objMailView->assignobj($objPage);
            $body = $objMailView->fetch('mail_templates/send_notification_request_info.tpl');

            $tosubject = 'メール件名';
            $objSendMail->setItem('', $tosubject, $body, $arrInfo['email03'], $arrInfo['shop_name'], $arrInfo['email03'], $arrInfo['email04'], $arrInfo['email04'], $bcc = '');
    ...
    private function searchRequestItemInfo($start, $end)
    {
        ...
        $sql = 'SELECT source_customer_id, source_name01, source_name02, source_kana01, source_kana02, source_email, source_email_mobile, source_nickname, source_product_id, source_product_name, source_sub_large_image1, count(distinct target_customer_id) as request_uu FROM '.
            '(SELECT * FROM dtb_customer_favorite_products WHERE create_date >= ? and create_date < ?) favorite_products  
