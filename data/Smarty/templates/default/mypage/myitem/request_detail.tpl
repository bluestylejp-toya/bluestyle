<section>
    ...
    <!--{* 受け取りが完了したら *}-->
    <div class="c-message--gray u-mb--0<!--{if $recieved !== true}--> --hidden<!--{/if}-->">受け取り完了</div>
    <!--{if $recieved !== true}-->

    <button type="button" href="/mypage/myitem/unselected_item.php?product_id=<!--{$tpl_product_id|h}-->" class="c-btn--mypage--unshipped">受け取りました</button>
    <!--{/if}-->
...
$(function(){
    ...
    //受け取りの処理
    $('.c-btn--mypage--unshipped').on('click', function(){
        $(this).fadeOut();
        $('.c-message--gray').fadeIn();
    });
})
</script>
